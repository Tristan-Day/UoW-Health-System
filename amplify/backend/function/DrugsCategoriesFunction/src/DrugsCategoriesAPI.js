...
class DrugsCategoriesAPI {
    ...
    static query = async function (req, res, setup) {
        ...
        try {
            ...
            if (Object.keys(req.query).includes(this.ID_COLUMN_NUMBER)) {
                ...
                query = await client.query('SELECT * FROM "system".drug_categories WHERE ID = ' + Number(req.query[this.ID_COLUMN_NUMBER]) + ';');
            ...
            if (paramLength == 0) {
                ...
                query = await client.query('SELECT * FROM "system".drug_categories;');
            ...
            if (paramLength > 0 && !Object.keys(req.query).includes(this.ID_COLUMN_NUMBER)) {
                ...
                let queryString = "SELECT * FROM \"system\".drug_categories WHERE ";
    ...
    static upsert = async function (req, res, setup) {
        ...
        try {
            if (req.body["ACTION_TYPE"] === "INSERT") {
                ...
                const queryString = `
                    INSERT INTO "system".drug_categories (DRUGNAME, STRENGTH, DOSAGE, AMOUNTDAILY, DRUG_CATEGORY_TYPE)
                    VALUES ($1, $2, $3, $4, $5)
                    `;
                const values = [
                    req.body["DRUGNAME"],
            ...
            if (req.body["ACTION_TYPE"] === "UPDATE") {
                ...
                const queryString = `
                UPDATE "system".drug_categories SET DRUGNAME = $1,STRENGTH = $2, DOSAGE = $3, AMOUNTDAILY = $4, DRUG_CATEGORY_TYPE = $5 WHERE ID = $6;
                `;
                const values = [
                    req.body["DRUGNAME"],
                    req.body["STRENGTH"],
    ...
    static delete = async function (req, res, setup) {
        ...
        try {
            ...
            const queryString = `
                    DELETE FROM "system".drug_categories WHERE ID = $1;
                    `;
...
module.exports = DrugsCategoriesAPI;