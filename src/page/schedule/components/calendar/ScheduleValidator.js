...
class ScheduleValidator {
  static async scheduleIsClear(startTime, duration, itemId) {
    ...
    const filteredScheduleItems = scheduleItems.filter(a => {
      ...
      return (
        proposedStartDate.getDate() === candidateDate.getDate() &&
    ...
    const itemsOverlapping = filteredScheduleItems.filter(a => {
      ...
      const proposedEndTime = new Date(
        proposedStartTime.getTime() + duration * 15 * 60000
      )

      const candidateStartTime = new Date(a.start_timestamp)
      const candidateEndTime = new Date(
        candidateStartTime.getTime() + a.estimated_duration_minutes * 15 * 60000
      )

      // start is in range
      const startInRange =
        candidateStartTime.getTime() <= proposedStartTime.getTime() &&
        candidateEndTime.getTime() >= proposedStartTime.getTime()

      // end is in range
      const endInRange =
        candidateEndTime.getTime() <= proposedEndTime.getTime() &&
        candidateEndTime.getTime() >= proposedStartTime.getTime()
