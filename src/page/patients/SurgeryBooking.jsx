...
import PropsConfirmationDialogue from './components/PropsConfirmationDialogue'
...
const SurgeryBooking = () => {
  ...
  async function handleSearch() {
    ...
    SurgeryBookingAPI.getSurgeryBooking()
      .then(result => {
        ...
        let filteredBookings = result.rows.filter(booking => {
          return booking.name.toLowerCase().indexOf(query.toLowerCase()) != -1
        })

        setContents(filteredBookings)
        setMessage(undefined)
      })
      .catch(() => {
        setContents([])
  ...
  async function handleDelete() {
    ...
    SurgeryBookingAPI.deleteSurgeryBooking(selection)
      .then(res => {
        console.log(res)
      ...
      .catch(error => {
        ...
        setMessage({
          text: 'Failed to delete surgery booking - Please try again later',
          severity: 'error',
          loading: false
        })
      })
  ...
  return (
    <div>
      ...
      <Box>
        <BreadcrumbGenerator />
        <Typography variant="h4">Surgery bookings</Typography>
